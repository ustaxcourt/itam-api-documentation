...
## Steps to import and migrate data between environments

1. (Step to import data in from SharePoint - Skip if already imported) Run the PowerQuery Import Dataflow from within the Test environment. This import file can also be created and used within Dev and Prod as needed for wherever your import needs to originate from. This can be done by taking the PowerQuery export and importing it to a Dataflow within the desired environment - then by mapping desired values to their target locations. As is - simply click the three dots and hit "Refresh"
