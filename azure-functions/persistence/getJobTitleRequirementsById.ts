...
import {
  AssetRequirement,
  restructureJobTitleRequirements,
} from './restructureJobTitleRequirements.js';
import { parseDataverseResponse } from './parseDataverseResponse.js';

// Dataverse always returns an array for value
type DataverseResponse<T> = {
  value?: T[];
};

// Uses the expected return from restructure (AssetRequirement) so we have a consistent data shape
export async function getJobTitleRequirementsById(
  id: string,
