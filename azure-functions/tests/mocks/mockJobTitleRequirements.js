export const mockDataverseResponseList = {
  '@odata.context':
    'https://org34e9a332.crm.dynamics.com/api/data/v9.2/$metadata#crf7f_ois_job_title_model_types(crf7f_modelmaximum,crf7f_modelminimum,crf7f_JobTitleAssetType,crf7f_ReferenceModel,crf7f_ReferenceModel(crf7f_name),crf7f_JobTitleAssetType(crf7f_minimumquanitity,crf7f_maximumquantity,crf7f_JobTitle,crf7f_JobTitle(crf7f_title),crf7f_AssetType(crf7f_name)))',
  value: [
    {
      '@odata.etag': 'W/"11194425"',
      crf7f_ois_job_title_model_typeid: 'cdfc4062-b0da-f011-8544-7c1e52177972',
      'crf7f_modelmaximum@OData.Community.Display.V1.FormattedValue': '3',
      crf7f_modelmaximum: 3,
      'crf7f_modelminimum@OData.Community.Display.V1.FormattedValue': '3',
      crf7f_modelminimum: 3,
      crf7f_ReferenceModel: {
        crf7f_name: 'CM568',
        crf7f_ois_asset_ref_modelid: '2a174d9a-69d7-f011-8544-000d3a35fa12',
      },
      crf7f_JobTitleAssetType: {
        crf7f_ois_job_title_asset_typeid:
          'bb954b00-afda-f011-8544-7c1e52177a24',
        'crf7f_minimumquanitity@OData.Community.Display.V1.FormattedValue': '4',
        crf7f_minimumquanitity: 4,
        'crf7f_maximumquantity@OData.Community.Display.V1.FormattedValue': '8',
        crf7f_maximumquantity: 8,
        crf7f_JobTitle: {
          crf7f_ois_job_titleid: 'b09cf686-30d5-f011-8544-7c1e52177972',
          crf7f_title: 'Administrative Specialist',
        },
        crf7f_AssetType: {
          crf7f_ois_asset_ref_typeid: 'ac40809b-69d7-f011-8544-000d3a5b5036',
          crf7f_name: 'Docking Station',
        },
      },
    },
    {
      '@odata.etag': 'W/"11194426"',
      crf7f_ois_job_title_model_typeid: '1b4579cc-f9d9-f011-8544-7c1e52177a24',
      'crf7f_modelmaximum@OData.Community.Display.V1.FormattedValue': '2',
      crf7f_modelmaximum: 2,
      'crf7f_modelminimum@OData.Community.Display.V1.FormattedValue': '2',
      crf7f_modelminimum: 2,
      crf7f_ReferenceModel: {
        crf7f_name: '727pm',
        crf7f_ois_asset_ref_modelid: '0f174d9a-69d7-f011-8544-000d3a35fa12',
      },
      crf7f_JobTitleAssetType: {
        crf7f_ois_job_title_asset_typeid:
          '174193a8-f9d9-f011-8544-7c1e52177a24',
        'crf7f_minimumquanitity@OData.Community.Display.V1.FormattedValue': '3',
        crf7f_minimumquanitity: 3,
        'crf7f_maximumquantity@OData.Community.Display.V1.FormattedValue': '2',
        crf7f_maximumquantity: 2,
        crf7f_JobTitle: {
          crf7f_ois_job_titleid: 'b09cf686-30d5-f011-8544-7c1e52177972',
          crf7f_title: 'Administrative Specialist',
        },
        crf7f_AssetType: {
          crf7f_ois_asset_ref_typeid: 'b540809b-69d7-f011-8544-000d3a5b5036',
          crf7f_name: 'Laptop',
        },
      },
    },
    {
      '@odata.etag': 'W/"11194427"',
      crf7f_ois_job_title_model_typeid: '3cf14ac6-f9d9-f011-8543-7c1e5257cc91',
      'crf7f_modelmaximum@OData.Community.Display.V1.FormattedValue': '1',
      crf7f_modelmaximum: 1,
      'crf7f_modelminimum@OData.Community.Display.V1.FormattedValue': '1',
      crf7f_modelminimum: 1,
      crf7f_ReferenceModel: {
        crf7f_name: '22U Open Framer Server Rack',
        crf7f_ois_asset_ref_modelid: '2e174d9a-69d7-f011-8544-000d3a35fa12',
      },
      crf7f_JobTitleAssetType: {
        crf7f_ois_job_title_asset_typeid:
          '174193a8-f9d9-f011-8544-7c1e52177a24',
        'crf7f_minimumquanitity@OData.Community.Display.V1.FormattedValue': '3',
        crf7f_minimumquanitity: 3,
        'crf7f_maximumquantity@OData.Community.Display.V1.FormattedValue': '2',
        crf7f_maximumquantity: 2,
        crf7f_JobTitle: {
          crf7f_ois_job_titleid: 'b09cf686-30d5-f011-8544-7c1e52177972',
          crf7f_title: 'Administrative Specialist',
        },
        crf7f_AssetType: {
          crf7f_ois_asset_ref_typeid: 'b540809b-69d7-f011-8544-000d3a5b5036',
          crf7f_name: 'Laptop',
        },
      },
    },
  ],
};

export const mockDataverseResponseItem = {
  '@odata.context':
    'https://org34e9a332.crm.dynamics.com/api/data/v9.2/$metadata#crf7f_ois_job_title_model_types(crf7f_modelmaximum,crf7f_JobTitleAssetType,crf7f_ReferenceModel,crf7f_ReferenceModel(crf7f_name),crf7f_JobTitleAssetType(crf7f_minimumquanitity,crf7f_maximumquantity,crf7f_JobTitle,crf7f_JobTitle(crf7f_title),crf7f_AssetType(crf7f_name)))',
  value: [
    {
      '@odata.etag': 'W/"11141663"',
      'crf7f_modelmaximum@OData.Community.Display.V1.FormattedValue': '1',
      crf7f_modelmaximum: 1,
      crf7f_ois_job_title_model_typeid: '3cf14ac6-f9d9-f011-8543-7c1e5257cc91',
      crf7f_ReferenceModel: {
        crf7f_name: '22U Open Framer Server Rack',
        crf7f_ois_asset_ref_modelid: '2e174d9a-69d7-f011-8544-000d3a35fa12',
      },
      crf7f_JobTitleAssetType: {
        crf7f_ois_job_title_asset_typeid:
          '174193a8-f9d9-f011-8544-7c1e52177a24',
        'crf7f_maximumquantity@OData.Community.Display.V1.FormattedValue': '2',
        crf7f_maximumquantity: 2,
        'crf7f_minimumquanitity@OData.Community.Display.V1.FormattedValue': '3',
        crf7f_minimumquanitity: 3,
        crf7f_JobTitle: {
          crf7f_title: 'Administrative Specialist',
          crf7f_ois_job_titleid: 'b09cf686-30d5-f011-8544-7c1e52177972',
        },
        crf7f_AssetType: {
          crf7f_ois_asset_ref_typeid: 'b540809b-69d7-f011-8544-000d3a5b5036',
          crf7f_name: 'Laptop',
        },
      },
    },
  ],
};
