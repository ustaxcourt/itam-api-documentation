...
describe('getJobTitleNameById', () => {
  ...
  it('should call dataverseCall with correct URL, method, and body', async () => {
    ...
    await getJobTitleNameById(jobTitleId);
  ...
  it('should parse the name from the dataverse response and return it', async () => {
    ...
    const result = await getJobTitleNameById(jobTitleId);
    expect(result).toEqual('Job Title 1');
  });
  ...
  it('should throw an InternalServerError if the response back from Dataverse is malformed', async () => {
    ...
    await expect(getJobTitleNameById(jobTitleId)).rejects.toThrow(
      InternalServerError,
    );
  });
  ...
  it('should throw NotFoundError when response has empty value array', async () => {
    ...
    await expect(getJobTitleNameById('job-title-123')).rejects.toThrow(
      NotFoundError,
    );
    await expect(getJobTitleNameById('job-title-123')).rejects.toThrow(
      'Resource not found',
